    ...
    <div className="min-h-screen bg-primary flex flex-col items-center justify-center px-6">
      ...
      <div className="mb-10 flex flex-col items-center gap-3">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-white">FamilyBudget</h1>
          <p className="text-sm text-muted mt-1">Семейный бюджет под контролем</p>
        </div>
      </div>

      {/* Card */}
      <div className={`w-full max-w-sm bg-card border border-border rounded-2xl p-6 transition-transform ${shake ? 'animate-shake' : ''}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <input
              ...
              className="w-full bg-primary border border-border rounded-xl px-4 py-3.5 text-white text-lg font-mono tracking-widest placeholder:text-muted/40 focus:outline-none focus:border-accent transition-colors"
              autoFocus
            />
            {error && (
          ...
          <button
            type="submit"
            disabled={!isReady}
            className="w-full flex items-center justify-center gap-2 bg-accent text-primary font-semibold py-3.5 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:bg-accent/90 active:scale-95"
          >
            Войти
