  ...
  return (
    <section className="bg-card border border-border rounded-2xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
        ...
        <p className="font-semibold text-ink text-sm">{t('income_distribution_title')}</p>
      </div>
      <div className="px-4 py-4 space-y-4">
        {(['mandatory', 'flexible', 'savings'] as const).map((key) => (
          <div key={key}>
            <div className="flex justify-between text-xs mb-1.5">
              <span className={`font-medium ${SLIDER_COLORS[key]}`}>{key === 'mandatory' ? t('mandatory_full') : key === 'flexible' ? t('flexible') : t('savings')}</span>
        ...
        <p className="text-xs text-muted text-center">{t('applies_to_new_incomes')}</p>
      </div>
    </section>
  );
}
