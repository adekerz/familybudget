    ...
    <>
      <section className="bg-card border border-border rounded-2xl overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          ...
          <p className="font-semibold text-ink text-sm">{t('category_limits_title')}</p>
        </div>
        <div className="divide-y divide-border">
          {categories.map((cat) => (
            <div key={cat.id} className="flex items-center justify-between px-4 py-2.5">
              ...
              >
                {cat.monthlyLimit ? formatMoney(cat.monthlyLimit) : t('no_limit')}
              </button>
            </div>
          ))}
        </div>
      ...
      <Modal isOpen={!!editId} onClose={() => setEditId(null)} title={t('limit_modal_title', { name: editCat?.name ?? '' })}>
        <div className="space-y-4">
          <p className="text-muted text-sm">{t('monthly_limit_label')}</p>
          ...
          <div className="flex gap-2">
            <Button variant="ghost" onClick={() => setEditId(null)} className="flex-1">{t('cancel')}</Button>
