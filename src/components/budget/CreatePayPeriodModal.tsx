...
export function CreatePayPeriodModal({ onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (diffDays > 1) {
      setError(
        t('period_future_date_error', { date: startDate }) +
        t('period_wrong_date_hint', { today })
      );
      return;
    }

    const sal = parseInt(salary);
    if (!sal || sal <= 0) { setError(t('error_salary_required')); return; }
    if (new Date(endDate) <= new Date(startDate)) { setError(t('error_end_after_start')); return; }
    setLoading(true);
